...
import { Send, X, MessageSquare, Bot, User, Terminal, AlertCircle } from 'lucide-react';
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-40 group flex items-center justify-center w-14 h-14 bg-dark border border-primary/60 text-primary rounded-full shadow-[0_0_15px_rgba(0,240,255,0.3)] hover:shadow-[0_0_30px_rgba(0,240,255,0.6)] hover:bg-primary hover:text-dark transition-all duration-300 ${isOpen ? 'scale-0' : 'scale-100'}`}
      >
      ...
      >
        ...
        <div className="bg-primary/10 p-3 flex justify-between items-center border-b border-primary/20 backdrop-blur-sm">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-mono text-xs text-primary tracking-[0.2em] uppercase">Gemini_Link_v2.5</span>
          </div>
        ...
        <div className="h-[450px] overflow-y-auto p-4 space-y-4 font-mono text-sm bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]">
          {messages.map((msg) => (
            ...
            >
              <div className={`flex-shrink-0 w-8 h-8 rounded-full border flex items-center justify-center ${msg.role === 'model' ? 'border-primary text-primary bg-primary/10' : 'border-slate-500 text-slate-300 bg-slate-800'}`}>
