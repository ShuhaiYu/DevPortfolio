...
const Hero: React.FC = () => {
  return (
    <section id="hero" className="relative min-h-screen flex flex-col justify-center items-center pt-20 pb-10 overflow-hidden bg-dark">
      
      {/* Cyber Grid Background */}
      <div className="absolute inset-0 w-full h-full perspective-[1000px] overflow-hidden -z-10">
      ...
      {/* Glowing Orbs - Adjusted for mobile to prevent overflow */}
      <div className="absolute top-[-10%] left-[-10%] w-[300px] sm:w-[600px] h-[300px] sm:h-[600px] bg-secondary/30 rounded-full filter blur-[80px] sm:blur-[120px] animate-blob opacity-40"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[250px] sm:w-[500px] h-[250px] sm:h-[500px] bg-primary/20 rounded-full filter blur-[80px] sm:blur-[120px] animate-blob animation-delay-2000 opacity-40"></div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10 flex flex-col items-center text-center">
        
        {/* Status Pill */}
        <div className="mb-6 sm:mb-8 inline-flex items-center space-x-2 px-4 py-2 rounded-full border border-primary/30 bg-primary/5 animate-slide-up backdrop-blur-sm" style={{animationDelay: '0.1s'}}>
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-primary"></span>
          </span>
          <span className="text-primary font-mono text-[10px] sm:text-xs tracking-widest uppercase">System Online</span>
        </div>
        ...
        {/* Subtitle */}
        <p className="mt-4 sm:mt-6 max-w-2xl text-slate-400 text-base sm:text-xl font-light animate-slide-up px-4" style={{animationDelay: '0.6s'}}>
          I'm <strong className="text-white">{OWNER_NAME}</strong>. {OWNER_TITLE}. <br className="hidden sm:block"/>
          Building high-performance interfaces for the <span className="text-accent font-mono">next web</span>.
        </p>

        {/* Action Buttons */}
        <div className="mt-8 sm:mt-12 flex flex-col sm:flex-row gap-4 sm:gap-6 w-full sm:w-auto px-4 animate-slide-up" style={{animationDelay: '0.8s'}}>
          <a 
            href="#projects" 
